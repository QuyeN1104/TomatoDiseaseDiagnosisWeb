...
with st.sidebar:
    ...
    st.write("Chọn loại bệnh bên dưới để xem ảnh test:")

    if os.path.exists(SAMPLE_DIR):
        # Lấy danh sách các thư mục con (tên bệnh) và sắp xếp A-Z
        sub_folders = sorted([f for f in os.listdir(SAMPLE_DIR) if os.path.isdir(os.path.join(SAMPLE_DIR, f))])

        if not sub_folders:
            st.warning(f"Thư mục '{SAMPLE_DIR}' rỗng hoặc không có thư mục con.")

        # Duyệt qua từng thư mục bệnh
        for folder_name in sub_folders:
            folder_path = os.path.join(SAMPLE_DIR, folder_name)

            # Tạo menu sổ xuống (Expander) cho từng loại bệnh
            with st.expander(f"📁 {folder_name}"):
                # Lấy danh sách file ảnh trong thư mục con này
                ...
                if sample_files:
                    ...
                    for i, file_name in enumerate(sample_files):
                        ...
                        # Hiển thị ảnh vào cột tương ứng
                        with cols[i % 2]:
                            try:
                                img_thumb = Image.open(file_path)
                                st.image(img_thumb,
                                         use_container_width=True)  # Streamlit mới dùng use_container_width thay cho use_column_width

                                # Tạo key duy nhất cho nút bấm bằng cách kết hợp tên folder + tên file
                                btn_key = f"btn_{folder_name}_{i}"

                                if st.button(f"Chọn", key=btn_key):
                                    st.session_state.current_image = img_thumb
                                    # Cập nhật source hiển thị rõ: Tên bệnh / Tên file
                                    st.session_state.image_source = f"Mẫu: {folder_name}/{file_name}"
                            except Exception as e:
                                st.error(f"Lỗi ảnh")
                else:
                    st.caption("Không có ảnh trong mục này.")
    else:
